...
| 路径 | 页面 | 作用 | 安全边界 |
|------|------|------|----------|
| `/scan` | 文件扫描 | 扫描临时文件、日志、缩略图缓存、Windows Update 缓存、空文件/空文件夹等整理候选项 | 只读扫描；不会移动文件 |
| `/duplicates` | 重复文件检测 | 扫描指定目录中的重复文件，按哈希分组并选择待整理项 | 只读扫描；选中项进入整理计划 |
| `/user-directory` | 用户目录扫描 | 分析 `%USERPROFILE%` 下的缓存、环境、构建产物、日志和大文件 | 只读扫描；用于空间判断和人工确认 |
| `/cleanup` | 整理计划 | 汇总扫描或重复检测结果，确认后执行整理 | 执行时先移动到 `ZeroTraceRecycle/`，保留审计记录 |
...
| `/registry` | 注册表扫描 | 检测无效路径、孤立 COM、无效卸载记录、服务、启动项和文件关联问题 | 整理前导出 `.reg` 备份；高风险和诊断项不会自动执行 |
